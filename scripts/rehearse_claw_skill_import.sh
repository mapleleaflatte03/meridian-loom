...
ROOT_DIR="${1:-/tmp/loom-clawskill-runtime}"
KERNEL_PATH="$(mktemp -d /tmp/loom-clawskill-kernel.XXXXXX)"
...
echo "== Meridian Loom // Clawfamily Skill Import Rehearsal =="
echo "root:   ${ROOT_DIR}"
...
cat > "${KERNEL_PATH}/kernel/runtimes.json" <<'EOF'
{
  "runtimes": {
    ...
    "meridian_loom": {
      "status": "experimental",
      "notes": "clawskill import rehearsal",
      "contract_compliance": {
        "agent_identity": null,
...
echo "--- Step 2: Import malware-triage from workspace skills ---"
...
echo "--- Step 4: Execute imported workspace skill ---"
"${LOOM}" action execute \
  --root "${ROOT_DIR}" \
  --kernel-path "${KERNEL_PATH}" \
...
echo "--- Step 5: Inspect imported-skill job ---"
"${LOOM}" job inspect --root "${ROOT_DIR}" --job-id "${JOB_ID}"
echo ""

echo "--- Step 6: Inspect parity ---"
